...
export const isDuplicateGenre = (newGenreName, existingGenres, newGenres) => {
  ...
  const existingDuplicate = existingGenres.find(
    (genre) => normalizeGenreName(genre.name) === normalizedNewName
  );
  ...
  const newDuplicate = newGenres.find(
    (genreName) => normalizeGenreName(genreName) === normalizedNewName
  );
